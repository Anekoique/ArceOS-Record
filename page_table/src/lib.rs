 ...
 */
use axconfig::{ASPACE_BITS, PAGE_SHIFT, PAGE_SIZE};
use axconfig::{align_down, align_offset, is_aligned};
use axconfig::{pfn_phys, phys_pfn, phys_to_virt, virt_to_phys};
...
impl PageTable<'_> {
    pub fn init(root_pa: usize, level: usize) -> Self {
        let table = unsafe {
            // Ensure the pointer is properly aligned for PTEntry
            assert!(
                root_pa != 0 && root_pa % core::mem::align_of::<PTEntry>() == 0,
                "Page table pointer must be non-null and properly aligned"
            );
